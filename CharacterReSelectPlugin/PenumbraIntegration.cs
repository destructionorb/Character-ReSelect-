...
namespace CharacterReSelectPlugin
{
    /// <summary>
    ...
    public class PenumbraIntegration : IDisposable
    {
        ...
        private void OnModAdded(string modDirectoryName)
        {
            try
            {
                ...
                if (modList != null && modList.TryGetValue(modDirectoryName, out var modDisplayName))
                {
                    ...
                    var plugin = CharacterReSelectPlugin.Plugin.Instance;
                    if (plugin?.modCategorizationCache != null)
                    {
                        var modType = CharacterReSelectPlugin.Windows.SecretModeModWindow.DetermineModType(modDirectoryName, modDisplayName, plugin);
        ...
        /// </summary>
        public ModConflictAnalysisResult AnalyzeModForDependenciesAndConflicts(string modDirectory, string modName, CharacterReSelectPlugin.Windows.ModType modType, Dictionary<string, bool> selectedMods)
        {
            ...
            if (modType == CharacterReSelectPlugin.Windows.ModType.Gear)
            {
            ...
            foreach (var selectedMod in selectedMods.Where(kvp => kvp.Value && kvp.Key != modDirectory))
            {
                ...
                if (modType == CharacterReSelectPlugin.Windows.ModType.Hair)
                {
